...
function checkMathAnswer() {
    ...
    switch (mathCurrentOperation) {
        case '+': correctAnswer = mathNum1 + mathNum2; break;
        case '-': correctAnswer = mathNum1 - mathNum2; break;
        case '×': correctAnswer = mathNum1 * mathNum2; break;
        case '÷': correctAnswer = mathNum1 / mathNum2; break;
    }
...
function generateSequence() {
    ...
    const sequenceHTML = currentSequence.map((num, index) => {
...
function generateComparisonQuestion() {
    ...
    document.querySelectorAll('.comparison-symbol-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.querySelectorAll('.comparison-symbol-btn').forEach(b => 
...
function checkComparisonAnswer(userAnswer) {
    if (!userAnswer) {
        document.getElementById('comparison-message').textContent = 'נא לבחור סימן השוואה';
...
function updateMathScore() {
    document.getElementById('math-score-display').textContent = 
        `תשובות נכונות: ${mathCorrectAnswers} מתוך ${mathQuestionsCount}`;
}

function updateSequenceScore() {
    document.getElementById('sequence-score-display').textContent = 
        `תשובות נכונות: ${sequenceCorrectAnswers} מתוך ${sequenceQuestionsCount}`;
}

function updateComparisonScore() {
    document.getElementById('comparison-score-display').textContent = 
        `תשובות נכונות: ${comparisonCorrectAnswers} מתוך ${comparisonQuestionsCount}`;
}
...
function showSummary(gameType) {
    const correctAnswers = gameType === 'math' ? mathCorrectAnswers :
                          gameType === 'sequence' ? sequenceCorrectAnswers :
                          comparisonCorrectAnswers;
    const questionsCount = gameType === 'math' ? mathQuestionsCount :
                          gameType === 'sequence' ? sequenceQuestionsCount :
                          comparisonQuestionsCount;
    const wrongAnswers = gameType === 'math' ? mathWrongAnswers :
                        gameType === 'sequence' ? sequenceWrongAnswers :
                        comparisonWrongAnswers;
...
window.onload = function() {
    ...
    document.querySelectorAll('#comparison-game .level-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            document.querySelectorAll('#comparison-game .level-btn').forEach(btn => {
                btn.classList.remove('active');
            });
            this.classList.add('active');
            generateComparisonQuestion();
    ...
    document.getElementById('math-check').addEventListener('click', checkMathAnswer);
    document.getElementById('math-next').addEventListener('click', generateMathQuestion);

    // מאזיני אירועים לכפתורי בדיקה וסדרה הבאה
    document.getElementById('sequence-check').addEventListener('click', checkSequenceAnswer);
    document.getElementById('sequence-next').addEventListener('click', generateSequence);

    // מאזיני אירועים לכפתורי בדיקה למשחק השוואה
    document.getElementById('comparison-check').addEventListener('click', checkComparisonAnswer);
    document.getElementById('comparison-next').addEventListener('click', generateComparisonQuestion);
    ...
    document.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            if (currentGame === 'math' && document.activeElement.id === 'answer') {
                checkMathAnswer();
            } else if (currentGame === 'sequence' && document.activeElement.id === 'sequence-answer') {
                checkSequenceAnswer();
            } else if (currentGame === 'comparison' && document.activeElement.id === 'comparison-answer') {
